...
export function getPostReviews(jwt) {
  ...
  return function(dispatch, getState) {
    ...
    const url = formURL(apiOrigin, `/v2/${apiEndpoints.postReviews}`)
    dispatch({
      type: types.postReviews.read.request,
      url,
    })
    const axiosConfig = {
      timeout: apiTimeout,
    ...
    return axios
      ...
      .then(res => {
        const successAction = buildSuccessActionFromRes(
          res,
          types.postReviews.read.success
        )
      ...
      .catch(error => {
        const failAction = failActionCreators.axios(
          error,
          types.postReviews.read.failure
        )
        dispatch(failAction)
